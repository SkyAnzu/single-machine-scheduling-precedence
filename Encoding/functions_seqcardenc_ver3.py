...
def solve_SAT(n, durations, ready_dates, deadlines, successors, verbose=False):
    ...
    s_to_a_clauses = 0
    for i in jobs:
        t_min = valid_starts[i][0]
        t_max = valid_starts[i][-1]
        p_i = durations[i]

        for t in range(ready_dates[i], deadlines[i]):
            a_lit = A[(i, t)]

            upper_t = t if t <= t_max else t_max
            upper_lit = L[(i, upper_t)]
            lower_t = t - p_i

            if lower_t < t_min:
                cnf.append([-a_lit, upper_lit])
                cnf.append([a_lit, -upper_lit])
                s_to_a_clauses += 2
                continue

            lower_lit = L[(i, lower_t)]
            cnf.append([-a_lit, upper_lit])
            cnf.append([-a_lit, -lower_lit])
            cnf.append([a_lit, -upper_lit, lower_lit])
            s_to_a_clauses += 3
    ...
    if verbose:
        ...
        print("Total clauses:", prec_clauses + l_clauses + cap_clauses + s_to_a_clauses + (1 if source_ready_anchor_clause else 0))
