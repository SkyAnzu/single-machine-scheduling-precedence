...
from common.schedule_utils import (
    all_ancestors_to_sink_endpoint_edges,
...
def _l_at_most(L, job, time_point, valid_starts):
    times = valid_starts[job]
    if time_point < times[0]:
        return False
    if time_point >= times[-1]:
        return True
    return L[(job, time_point)]


def _not_l_at_most(L, job, time_point, valid_starts):
    value = _l_at_most(L, job, time_point, valid_starts)
    if value is True:
        return False
    if value is False:
        return True
...
def _append_simplified_clause(cnf, base_literals, optional_literals):
    clause = list(base_literals)
    for lit in optional_literals:
        if lit is True:
            return False
        if lit is False:
            continue
...
def solve_SAT(n, durations, ready_dates, deadlines, successors, verbose=False, sat_solver_name="g421"):
    ...
    extra_edges = all_ancestors_to_sink_endpoint_edges(n, successors)
    extended_successors = extend_successors(n, successors, extra_edges)
    original_edge_count = sum(len(successors.get(job, [])) for job in jobs)
    extended_edge_count = sum(len(extended_successors.get(job, [])) for job in jobs)
    ...
    if verbose:
        print(f"Created S variables: {len(S)}. Next var id = {var_counter}")
    ...
    for i_index, i in enumerate(jobs):
        for j in jobs[i_index + 1:]:
            for t_i in valid_starts[i]:
                ...
                appended = _append_simplified_clause(
                    cnf,
                    [-S[(i, t_i)]],
                    [left, right],
                )
    ...
    prec_clauses = 0
    for i in jobs:
        for j in extended_successors.get(i, []):
    ...
    if verbose:
        print("Endpoint strategy: all-ancestors-to-sink")
        print("Original precedence edges:", original_edge_count)
        print("Endpoint edges added:", len(extra_edges))
        print("Extended precedence edges:", extended_edge_count)
...
def incremental_SAT_Lmax(durations, due_dates, S, L, cnf, UB, sol_file, valid_starts, verbose=False, sat_solver_name="g421"):
    solver = Solver(name=sat_solver_name, bootstrap_with=cnf)
    iteration_count = 0
